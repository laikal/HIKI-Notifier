...
namespace HikiNotifier.UI
{
    internal static class LogoResources
    {
        ...
        private static readonly Lazy<Image> youtube = new Lazy<Image>(() => Load("YOUTUBE"));
        ...
        {
            if (platform == PlatformType.YouTube) return youtube.Value;
            if (platform == PlatformType.Rplay) return rplay.Value;
            if (platform == PlatformType.Twitch) return twitch.Value;
            return chzzk.Value;
        }

        public static PlatformType[] TestPlatforms => new[] { PlatformType.Chzzk, PlatformType.Rplay, PlatformType.YouTube, PlatformType.Twitch };
