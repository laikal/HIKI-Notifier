...
namespace HikiNotifier.UI
{
    internal sealed class ProfileListView : ListView
    {
        ...
        private void DrawCell(object sender, DrawListViewSubItemEventArgs e)
        {
            ...
            var streamer = e.Item.Tag as StreamerProfile;
            if (profile == null && streamer == null) return;
            using (var background = new SolidBrush(e.Item.Selected ? UiTheme.Selected :
                streamer != null ? UiTheme.Header : UiTheme.Surface))
                e.Graphics.FillRectangle(background, e.Bounds);
            using (var line = new Pen(UiTheme.Divider))
                e.Graphics.DrawLine(line, e.Bounds.Left, e.Bounds.Bottom - 1, e.Bounds.Right, e.Bounds.Bottom - 1);
            ...
            {
                using (var headerFont = new Font(Font, FontStyle.Bold))
                    TextRenderer.DrawText(e.Graphics, e.SubItem.Text ?? "", headerFont, bounds,
                        e.ColumnIndex == 4 ? UiTheme.Muted : UiTheme.Text,
            ...
            if (e.ColumnIndex == 1)
            {
                ...
                color = profile.Platform == PlatformType.YouTube ? UiTheme.Muted :
                    profile.State == LiveState.Live ? UiTheme.Live :
                    profile.State == LiveState.Offline ? UiTheme.Offline : UiTheme.Unknown;
                if (profile.Platform != PlatformType.YouTube)
                {
